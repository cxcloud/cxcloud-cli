...
exports.ensureDeployEnvironment = async () => {
  const paths = ['./package.json', './.cxcloud.yaml'];
...
exports.readDeploymentManifest = async () => {
  ...
  if (!deployment || !deployment.image || !deployment.image.version) {
    throw new Error('You have en error in your `.cxcloud.yaml` file.');
  }
